...
class Bitset
{
    ...
    /**
     * Counts the number of elements in a Bitset by using getBit to check
     * what bits are set
     */
 ...
 boolean get = true;
 int count = 0;
 while (get && count <= this.cardinality()){
     if ((this.member(count)&& setB.member(count))||
             (!this.member(count)&&!setB.member(count))){
         count++;
     }
     else get = false;
 }
